...
const AllArticles = () => {
  ...
  const approvedArticle = data?.data.filter(
    ...
    <div className="mt-20">
      ...
      {!isLoading &&
        approvedArticle.map((articleCard) => (
          <div
            key={articleCard._id}
            className="px-4 py-8 shadow-lg max-w-[350px] font-sans rounded-xl space-y-6 my-20 mx-auto"
          >
            ...
            <div className="text-center w-[85%] mx-auto font-semibold space-y-2">
              <h6 className="text-sm md:text-base lg:text-lg">
                {truncateText(articleCard?.description, MAX_WORDS)}
              </h6>
              <p className="text-gray-400 text-xs md:text-sm font-semibold">
                {articleCard?.publisher}
              </p>
            </div>
            <div className="flex items-center justify-center flex-wrap gap-6 text-sm md:text-base">
              <Link
                to={`/article/details/${articleCard.title}/${articleCard._id}`}
                className="px-4 py-2 rounded-lg bg-[#49B2FF] hover:bg-sky-600 duration-300 hover:scale-105 text-white font-semibold font-sans"
              >
                Details
              </Link>
